...
export class TrackedBrokerService {
  ...
  /**
   * Get positions with error logging
   */
  async getPositions(brokerAccountId: string): Promise<any> {
    const startTime = performance.now();
    const traceId = TraceContext.getTraceId() || traceIdService.generateTraceId();
    ...
    const context: BrokerOperationContext = {
      ...
      requestDetails: {
        method: 'GET',
        url: `/api/broker/positions`,
        requestId: traceId
      }
    };

    try {
      await traceIdService.addOperation(traceId, 'GET_POSITIONS', 'BROKER_SERVICE');

      const result = await this.brokerService.getPositions(brokerAccountId);
      const duration = performance.now() - startTime;
      ...
      if (result.success) {
        ...
        await brokerErrorLoggingService.logBrokerError(
          result.message || 'Failed to get positions',
          new Error(result.message || 'Unknown positions error'),
          {
            ...context,
            requestDetails: {
              ...context.requestDetails,
  ...
  async getQuotes(symbols: string[]): Promise<any> {
    ...
    const context: BrokerOperationContext = {
      ...
      requestDetails: {
        method: 'POST',
        url: `/api/broker/quotes`,
        requestId: traceId
      }
    };

    try {
      await traceIdService.addOperation(traceId, 'GET_QUOTES', 'BROKER_SERVICE');

      // const result = await this.brokerService.getQuotes(symbols); // Method doesn't exist, use getQuote
      const result = { success: false, message: 'getQuotes not implemented, use getQuote instead' };
      const duration = performance.now() - startTime;

      if (result.success) {
        await brokerErrorLoggingService.logBrokerSuccess(
          `Quotes retrieved for ${symbols.length} symbols`,
          {
            ...context,
            requestDetails: {
              ...context.requestDetails,
        ...
        await brokerErrorLoggingService.logBrokerError(
          result.message || 'Failed to get quotes',
          new Error(result.message || 'Unknown quotes error'),
          {
            ...context,
            requestDetails: {
              ...context.requestDetails,
      ...
      await brokerErrorLoggingService.logBrokerError(
        `Failed to get quotes: ${error.message}`,
        error,
        {
          ...context,
          requestDetails: {
