...
describe('ShoonyaSymbolConverter', () => {
  ...
  describe('getBrokerName', () => {
    it('should return shoonya as broker name', () => {
      expect(converter.getBrokerName()).toBe('shoonya');
    });
  });
  ...
  describe('convertToBrokerFormat - Options', () => {
    it('should convert NSE option symbol correctly with exchange mapping to NFO', () => {
      const symbol: StandardizedSymbol = {
        id: 'nifty-option-ce',
      ...
      expect(result.exchange).toBe('NFO'); // NSE maps to NFO for derivatives
    ...
    it('should convert NFO option symbol correctly', () => {
      ...
      expect(result.tradingSymbol).toBe('NIFTY25JAN21000PE');
      expect(result.exchange).toBe('NFO'); // NFO remains NFO
      expect(result.segment).toBe('FO');
    });

    it('should convert BSE option symbol correctly with exchange mapping to BFO', () => {
      const symbol: StandardizedSymbol = {
        id: 'sensex-option-ce',
        displayName: 'SENSEX 70000 CE 30 JAN 25',
        tradingSymbol: 'SENSEX25JAN70000CE',
        instrumentType: 'OPTION',
        exchange: 'BSE',
        segment: 'FO',
        underlying: 'SENSEX',
      ...
      expect(result.tradingSymbol).toBe('SENSEX25JAN70000CE');
      expect(result.exchange).toBe('BFO'); // BSE maps to BFO for derivatives
      expect(result.segment).toBe('FO');
    });
  ...
  describe('convertToBrokerFormat - Futures', () => {
    it('should convert NSE future symbol correctly with exchange mapping to NFO', () => {
      const symbol: StandardizedSymbol = {
        id: 'nifty-future',
    ...
    it('should convert MCX future symbol correctly', () => {
      ...
      expect(result.tradingSymbol).toBe('GOLD25JANFUT');
      expect(result.exchange).toBe('MCX'); // MCX remains MCX
      expect(result.segment).toBe('FO');
    });

    it('should convert BFO future symbol correctly', () => {
      const symbol: StandardizedSymbol = {
        id: 'sensex-future',
        displayName: 'SENSEX FUT 30 JAN 25',
        tradingSymbol: 'SENSEX25JANFUT',
        instrumentType: 'FUTURE',
        exchange: 'BFO',
        segment: 'FO',
        underlying: 'SENSEX',
      ...
      expect(result.tradingSymbol).toBe('SENSEX25JANFUT');
      expect(result.exchange).toBe('BFO'); // BFO remains BFO
      expect(result.segment).toBe('FO');
    });
  ...
  describe('convertToBrokerFormat - Error Cases', () => {
    it('should throw error for unsupported exchange', () => {
      ...
      expect(() => converter.convertToBrokerFormat(symbol))
        .toThrow('Shoonya does not support exchange: UNSUPPORTED');
    });
